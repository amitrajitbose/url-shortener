...
function App() {
  ...
  const createUrl = async (event) => {
    ...
    const url = '/api/v1/';
    const custom = !!event.target[1].value;
    const payload = {
            ...
            <form className="url-form" ref={form} onSubmit={createUrl}>
              <div className="url-form-header">
                {submitting
                  ? <Loader />
                  : (
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#323b40" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="feather feather-link-2">
